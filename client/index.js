!(function () {
    ...
    request.on('error', err => {
      ...
      request.on('end', () => {
        ...
        switch (userdata.messageType) {
          case 'adjustment':
            var functionName = `${userdata.actionType}(${userdata.payload})`;
            csInterface.evalScript(functionName, result => {
              response.statusCode = 200; // 성공 상태 코드
              response.setHeader('Content-Type', 'text/plain'); // header 설정
              response.write(result); // body에 정보 탑재
              response.end(); // 정보 탑재 후 브라우저로 전송
            });
            // csInterface.evalScript(functionName);
            break;
          case 'adjustmentLayer':
            var functionName = `${userdata.actionType}("${userdata.typeKey}",${userdata.payload})`;
            csInterface.evalScript(functionName, result => {
              response.statusCode = 200; // 성공 상태 코드
              response.setHeader('Content-Type', 'text/plain'); // header 설정
              response.write(result); // body에 정보 탑재
            ...
            break;
          case 'command':
            var functionName = `${userdata.actionType}("${userdata.typeKey}")`;
            csInterface.evalScript(functionName, result => {
              response.statusCode = 200; // 성공 상태 코드
              response.setHeader('Content-Type', 'text/plain'); // header 설정
              response.write(result); // body에 정보 탑재
            ...
            break;
          case 'modifyProperty':
            var functionName = userdata.actionType + '(' + JSON.stringify(userdata) + ')';
            csInterface.evalScript(functionName, result => {
              response.statusCode = 200; // 성공 상태 코드
              response.setHeader('Content-Type', 'text/plain'); // header 설정
              response.write(result); // body에 정보 탑재
  ...
  function createPortFile(portNumber) {
